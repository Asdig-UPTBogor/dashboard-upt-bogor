...
export default function TrendGangguanPage() {
    ...
    const trendComboChart = useMemo(() => {
        ...
        return {
            ...
            legend: {
                data: [showPHT ? "Penghantar" : "", showTRF ? "Trafo" : "", "Rata-rata (3 Tahun)"].filter(Boolean), bottom: 0,
            ...
            series: [
                ...
                {
                    name: "Rata-rata (3 Tahun)", type: "line" as const,
    ...
    // ═══════════════════════════════════════════
    // CHART 2: Radar/Spider — Kategori Distribution
    // ═══════════════════════════════════════════
    const radarChart = useMemo(() => {
        const counts: Record<string, number> = {};
        filtered.forEach(r => { const k = r["Kategori"] || "N/A"; counts[k] = (counts[k] || 0) + 1; });
        ...
        return {
            ...
            radar: {
                indicator: sorted.map(([name, val]) => ({ name: name.length > 16 ? name.slice(0, 14) + "..." : name, max: maxVal * 1.2 })),
                shape: "polygon" as const, radius: "65%", center: ["50%", "52%"],
                axisName: { color: "#d4d4d8", fontSize: 9 },
                splitArea: { areaStyle: { color: ["rgba(129,140,248,0.02)", "rgba(129,140,248,0.05)", "rgba(129,140,248,0.02)", "rgba(129,140,248,0.05)"] } },
            ...
            series: [{
                type: "radar" as const,
                data: [{
                    value: sorted.map(s => s[1]),
                    name: "Kategori Gangguan",
                    areaStyle: { color: { type: "linear" as const, x: 0, y: 0, x2: 1, y2: 1,
                        colorStops: [{ offset: 0, color: "rgba(129,140,248,0.35)" }, { offset: 1, color: "rgba(192,132,252,0.1)" }] } },
                    lineStyle: { color: P.indigo, width: 2 },
                    itemStyle: { color: P.indigo, borderWidth: 2, borderColor: "#1a1a2e" },
                    emphasis: { areaStyle: { color: "rgba(129,140,248,0.45)" } },
                }],
            }],
        ...
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <Card className="lg:col-span-5">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm flex items-center gap-2">
                            <AlertTriangle className="h-4 w-4 text-primary" /> Distribusi Kategori Gangguan
                            <Badge variant="secondary" className="ml-auto text-[9px]">Radar</Badge>
                        </CardTitle>
                    </CardHeader>
                    <CardContent><ReactECharts option={radarChart} style={{ height: 320 }} /></CardContent>
                </Card>
