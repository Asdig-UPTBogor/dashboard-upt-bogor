...
export default function AssetTransmisiPage() {
    const [rawData, setRawData] = useState<AssetRecord[]>([]);
    ...
    useEffect(() => {
        fetch("/api/asset-transmisi")
            .then(r => {
                if (!r.ok) throw new Error("Gagal mengambil data Asset Transmisi");
                return r.json();
            })
            .then(d => {
                setRawData(d.data || []);
    ...
    // Bar Chart Data (Total Penghantar by ULTG)
    ...
    // Pie Chart Data (Operasi/Tidak Operasi)
    const operasiChartOption = useMemo(() => {
        return {
            backgroundColor: "transparent",
            tooltip: { trigger: "item", formatter: "{b}: {c} Tower ({d}%)" },
            legend: { top: "bottom", textStyle: { color: "#888", fontSize: 10 } },
            series: [{
                name: "Status Operasi",
                type: "pie",
                radius: ["40%", "70%"],
    ...
    }, [countOperasi, countTidakOperasi, activeOperasiStatus]);
    ...
    const visibleHeaders = useMemo(() => {
        ...
        return headers.filter(h => {
            ...
            // Hide KETERANGAN column
            if (upperH === "KETERANGAN") {
                return false;
            }
            return true;
        });
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b pb-4">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <Badge variant="outline" className="text-[10px]">
                        <RefreshCw className="h-3 w-3 mr-1" /> Data Terkini
                    </Badge>
                </div>
            </div>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        ...
                        <div className="relative flex-1 max-w-sm">
                            ...
                            <Input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                                placeholder="Cari tower, penghantar, dll..."
                    ...
                    <div className="overflow-x-auto rounded-md border mt-3">
                        <Table>
                            <TableHeader>
                                ...
                                <TableRow className="bg-muted/50 hover:bg-muted/50">
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center border-r">Usia</TableHead>
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center border-r">Operasi</TableHead>
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center border-r">Jenis</TableHead>
                                    ...
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center border-r">Usia</TableHead>
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center border-r">Operasi</TableHead>
                                    <TableHead className="whitespace-nowrap text-[10px] font-semibold py-1 text-center">Jenis</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
