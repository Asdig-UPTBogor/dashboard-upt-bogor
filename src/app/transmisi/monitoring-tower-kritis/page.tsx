...
export default function MonitoringTowerKritisPage() {
    ...
    useEffect(() => {
        fetch("/api/monitoring-tower-kritis")
            .then(r => r.json())
            .then(json => {
    ...
    const giList = useMemo(() => {
        ...
        if (filterULTG || activeULTG) {
            ...
            src = src.filter(r => r["MASTER ULTG"] === u || r["ULTG"] === u);
        }
    ...
    const filtered = useMemo(() => {
        ...
        if (currentULTG) result = result.filter(r => r["Master ULTG"] === currentULTG || r["ULTG"] === currentULTG);
    ...
    const venomData = useMemo(() => {
        return filtered.filter(r => (r["ONLINE/OFFLINE"] || "").toUpperCase().includes("ONLINE"));
    }, [filtered]);
    ...
    const visibleHeaders = useMemo(() => {
        const excludeCols = ["MASTER ULTG", "MASTER GARDU INDUK"];
        let hasNoCol = false;
        return headers.filter(h => {
            const upperH = (h || "").toUpperCase();
            if (excludeCols.includes(upperH)) return false;
                            ...
                            <TableBody>
                                {paginatedVenomData.map((r, i) => (
                                    <TableRow key={i} className="hover:bg-muted/50 transition-colors">
                                        ...
                                        <TableCell className="text-xs font-mono font-medium text-emerald-400">{r["ID"] || "-"}</TableCell>
                                        <TableCell className="text-[10px] whitespace-nowrap">{r["ULTG"] || "-"}</TableCell>
                                        <TableCell className="text-[10px] whitespace-nowrap">{r["GARDU INDUK"] || "-"}</TableCell>
                                        <TableCell className="text-[10px] max-w-[200px] truncate" title={r["PENGHANTAR"]}>{r["PENGHANTAR"] || "-"}</TableCell>
                                        <TableCell className="text-[10px] whitespace-nowrap">{r["NO TOWER"] || "-"}</TableCell>
                                        <TableCell className="text-[10px] whitespace-nowrap">
                                            {r["LINK VENOM"] ? (
                                                <a href={r["LINK VENOM"]} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-blue-400 hover:text-blue-300 hover:underline">
                            ...
                            <TableHeader>
                                <TableRow>
                                    ...
                                    {visibleHeaders.map((h, i) => {
                                        if (h.toUpperCase() === "NO" || h.toUpperCase() === "NO.") return null; // Skip if we use custom No
