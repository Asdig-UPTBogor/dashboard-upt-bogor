 ...
 * /presentation/ce
 *
 * Slide deck Common Enemy (CE) UPT Bogor — container selaras deck Program Kerja (golden).
 * - Multi-slide: Cover · Ringkasan · Transmisi (HARJAR) · Gardu Induk (HARGI) · Proteksi (HARPRO)
 * - Keyboard nav (← → Space PgUp/PgDn Home End)
 * - URL sync ?slide=N (deep link)
 * - Zoom Fit/Manual + export PNG (1×–4×) + fullscreen "Presentasi"
 *
 * Data (zero hardcode): useSnapshot("ce_summary") = angka RESMI per program,
 * useSnapshot("ce_detail", {bidang}) ×3 = split ULTG + non-target (basis target-only),
 * useSnapshot("meta") = ROW transmisi + tanggal snapshot. Semua di-agg sekali di
 * container (buildBidangDeck), di-pass ke slide via props. Snapshot belum sanitize
 * → agregasi defensive (lihat _lib/ce-deck.ts).
 */
...
import { CeCoverSlide } from "./_slides/CeCoverSlide";
import { CeRingkasanSlide } from "./_slides/CeRingkasanSlide";
import { CeBidangSlide } from "./_slides/CeBidangSlide";

/* ─────────── Deck data shared across slides ─────────── */
interface DeckData {
    decks: Record<CeBidang, BidangDeck> | null;
    /** ROW Right of Way transmisi — dari meta tx_row_*. */
    row: { target: number; realisasi: number };
    updatedDate: string;
    loading: boolean;
    error: string | null;
}
...
interface SlideDef {
    key: string;
    title: string;
    render: (props: { slideNo: number; total: number; data: DeckData }) => React.ReactNode;
}
...
const SLIDES: SlideDef[] = [
    {
        key: "cover",
        title: "Cover",
        render: ({ slideNo, total, data }) => (
            <CeCoverSlide slideNo={slideNo} total={total} decks={data.decks} updatedDate={data.updatedDate} />
        ),
    },
    {
        key: "ringkasan",
        title: "Ringkasan CE UPT Bogor",
        render: ({ slideNo, total, data }) => (
            <CeRingkasanSlide
                slideNo={slideNo}
                total={total}
    ...
    {
        key: "transmisi",
        title: "Transmisi (HARJAR)",
        render: ({ slideNo, total, data }) => (
            <CeBidangSlide
                slideNo={slideNo}
                total={total}
                deck={data.decks?.transmisi ?? null}
                row={data.row}
                loading={data.loading}
                error={data.error}
            />
    ...
    {
        key: "gardu-induk",
        title: "Gardu Induk (HARGI)",
        render: ({ slideNo, total, data }) => (
            <CeBidangSlide
                slideNo={slideNo}
                total={total}
                deck={data.decks?.gardu_induk ?? null}
                loading={data.loading}
                error={data.error}
            />
        ),
    ...
    {
        key: "proteksi",
        title: "Proteksi (HARPRO)",
        render: ({ slideNo, total, data }) => (
            <CeBidangSlide
                slideNo={slideNo}
                total={total}
                deck={data.decks?.proteksi ?? null}
                loading={data.loading}
                error={data.error}
            />
        ),
...
export default function CeDeckPage() {
    return (
...
function DeckContent() {
    ...
    /* ─── Snapshot data (fetch once, share ke semua slide) ─── */
    const summary = useSnapshot<CeSummaryRow>("ce_summary");
    const meta = useSnapshot<MetaRow>("meta");
    const dTx = useSnapshot<CeDetailRow>("ce_detail", { bidang: "transmisi" });
    const dGi = useSnapshot<CeDetailRow>("ce_detail", { bidang: "gardu_induk" });
    const dPr = useSnapshot<CeDetailRow>("ce_detail", { bidang: "proteksi" });

    const data: DeckData = useMemo(() => {
        const loading = summary.loading || meta.loading || dTx.loading || dGi.loading || dPr.loading;
        const error = summary.error || dTx.error || dGi.error || dPr.error || meta.error;
        const decks = summary.rows.length > 0
            ? {
                  transmisi: buildBidangDeck("transmisi", summary.rows, dTx.rows),
                  gardu_induk: buildBidangDeck("gardu_induk", summary.rows, dGi.rows),
                  proteksi: buildBidangDeck("proteksi", summary.rows, dPr.rows),
              }
            : null;
        const row = {
            target: getMetaNum(meta.rows, "tx_row_target"),
            realisasi: getMetaNum(meta.rows, "tx_row_realisasi"),
        };
        const updatedDate = getUpdatedDate(meta.rows);
        return { decks, row, updatedDate, loading, error };
    }, [
        summary.rows, summary.loading, summary.error,
        meta.rows, meta.loading, meta.error,
        dTx.rows, dTx.loading, dTx.error,
        dGi.rows, dGi.loading, dGi.error,
        dPr.rows, dPr.loading, dPr.error,
    ]);
    ...
    async function downloadPng(pixelRatio: number) {
        ...
        try {
            ...
            a.href = dataUrl;
            a.download = `deck-ce-${current.key}-${stamp}-${pixelRatio}x.png`;
            a.click();
        } finally {
        ...
        <>
            ...
            <div className="slide-stage-outer" style={{ width: 1920 * scale, height: 1080 * scale }}>
                <div className="slide-stage" style={{ transform: `scale(${scale})` }}>
                    <div ref={slideRef}>
                        {current.render({ slideNo: idx + 1, total: SLIDES.length, data })}
                    </div>
                </div>
            </div>
        </>
