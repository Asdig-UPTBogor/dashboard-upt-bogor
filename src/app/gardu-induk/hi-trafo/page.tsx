...
const STATUS_HI_COLORS: Record<string, { color: string }> = {
    "VERY GOOD": { color: C.emerald },
    "GOOD": { color: C.green },
    "FAIR": { color: C.amber },
    "POOR": { color: C.orange },
    "VERY POOR": { color: C.rose },
};

const CRITICALITY_COLORS: Record<string, string> = {
    "SANGAT TINGGI": C.rose,
    "TINGGI": C.orange,
    "SEDANG": C.amber,
    "RENDAH": C.emerald,
};
...
export default function HiTrafoPage() {
    ...
    const rows: TrafoRow[] = useMemo(() =>
        rawData.map(r => ({
            ultg: r["Master ULTG"] || "",
            gi: r["Gi"] || r["Master Gardu Induk"] || "",
            bay: r["Bay"] || r["Master Bay"] || "",
            mtu: r["MTU"] || "",
            phasa: r["Phasa"] || "",
            mva: r["MVA"] || "",
            merek: r["Merek"] || "",
            tipe: r["Tipe"] || "",
            serialId: r["Serial Id"] || "",
            tahunBuat: r["Tahun Buat"] || "",
            tahunOperasi: r["Tahun Operasi"] || "",
            criticalityGi: r["Criticality Gi"] || "",
            prioritas: r["Prioritas Penggantian"] || "",
            justifikasi: r["Justifikasi Prioritas"] || "",
            statusUsia: r["Status Usia"] || "",
            rencana: r["Rencana"] || "",
            statusHi: (r["Status Hi"] || "").toUpperCase().trim(),
            nilaiHi: parseFloat(r["Nilai Hi"] || "0") || 0,
    ...
    // 1. Status HI Distribution (donut)
    const statusDonut = useMemo(() => {
        ...
        const data = order.filter(s => distMap[s]).map(name => ({
            ...
            itemStyle: { color: STATUS_HI_COLORS[name]?.color || C.indigo },
        }));
        const total = data.reduce((s, d) => s + d.value, 0);
        return {
            ...echartBase,
            tooltip: { trigger: "item" as const, backgroundColor: "rgba(15,15,30,0.95)", borderColor: "rgba(129,140,248,0.3)", textStyle: { color: "#e4e4e7" }, formatter: "{b}: {c} trafo ({d}%)" },
            legend: {
            ...
            graphic: [{
                ...
                style: { text: "trafo", fontSize: 11, fill: "#71717a", textAlign: "center" as const },
            }],
            series: [{
    ...
    const critDonut = useMemo(() => {
        ...
        const data = Object.entries(distMap).map(([name, value]) => ({
            name, value,
            itemStyle: { color: CRITICALITY_COLORS[name] || C.purple },
        }));
        const total = data.reduce((s, d) => s + d.value, 0);
        return {
            ...echartBase,
    ...
    const prioritasChart = useMemo(() => {
        ...
        return {
            ...
            series: [{
                ...
                label: { show: true, position: "top" as const, fontSize: 12, fontWeight: "bold" as const, color: "#e4e4e7" },
            }],
            animationDuration: 1000,
        };
    }, [filtered]);
