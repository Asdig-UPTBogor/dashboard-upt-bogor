...
export function CEGarduIndukContent({ sheetData, giToUltgMap = {} }: { sheetData: any; giToUltgMap?: Record<string, string> }) {
     ...
     *  rowsForUltg    = rawRows filtered by [program, doneFilter]  → ULTG donut
     *  rowsForProgram = rawRows filtered by [ultg, doneFilter]     → Program donut
     *  baseRows       = rawRows filtered by [ultg, program]        → close/open bar
     *  filteredRows   = rawRows filtered by [all + doneFilter]     → tabel (existing)
    ...
    const rowsForUltg = useMemo(() => {
        ...
        if (doneFilter !== null) r = r.filter(x => isDone(x) === doneFilter);
        return r;
    }, [rawRows, selectedProgram, doneFilter]);
    ...
    const rowsForProgram = useMemo(() => {
        ...
        if (doneFilter !== null) r = r.filter(x => isDone(x) === doneFilter);
        return r;
    }, [rawRows, selectedUltg, doneFilter, getUltg]);
    ...
    const baseRows = useMemo(() => {
        ...
        if (selectedClosing) r = r.filter(x => (x[H.CLOSING] || "").toUpperCase() === selectedClosing);
        return r;
    }, [rawRows, selectedUltg, selectedProgram, selectedGI, selectedClosing, getUltg]);
    ...
    const donutData = useMemo(() => {
        ...
        return { ultg, program, ultgCounts, programCounts };
    }, [rowsForUltg, rowsForProgram, getUltg]);
    ...
    const comboChartOption = useMemo(() => {
        ...
        return {
            ...
            legend: {
                ...
                textStyle: { color: ec.text, fontSize: ECHART_FONT.label },
            ...
            series: [
                ...barSeries,
                {
                    ...
                    data: sisaData,
                    lineStyle: { width: 3, color: COLOR_SELESAI },
                    itemStyle: { color: COLOR_SELESAI, borderWidth: 2 },
    ...
    const donutProgram = useMemo(() => {
        ...
        const selectedAlias = selectedProgram ? toTitleCase(selectedProgram) : null;
        return mkDonut(data, selectedAlias);
    }, [donutData, mkDonut, selectedProgram]);
    ...
    const onClickProgram = useMemo(() => ({
        click: (p: { name?: string }) => {
            ...
            const original = donutData.program.find(k => k.toLowerCase() === lower) || p.name;
        ...
        <div className="flex flex-col gap-3 relative">
            {hasFilter && (
                <div className="absolute -top-10 right-0 flex items-center gap-1.5 flex-wrap z-10">
                    ...
                    {selectedClosing && <Badge variant="outline" className={`ds-data cursor-pointer gap-1 hover:bg-destructive/20 border-border bg-muted text-muted-foreground`} onClick={() => setSelectedClosing(null)}><X className="h-2.5 w-2.5" />{selectedClosing}</Badge>}
            ...
            {/* Program CE Gardu Induk + Summary — identik pattern CE Transmisi */}
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-3">
                <StatusKpiBar1
                    title="Program CE Gardu Induk"
                    items={programStats}
                    activeStatus={selectedProgram}
                    onStatusFilter={setSelectedProgram}
                    close={progressStats.selesai}
                    open={progressStats.belum}
                    activeDone={doneFilter}
                    onDoneFilter={setDoneFilter}
                    shadowColor={ec.shadow}
                />
                <SummaryCard1
                    title="Total CE Gardu Induk"
                    total={totalCE}
                    items={[
            ...
            {false && <div className="rounded-md border border-border hover:border-primary/10 ds-transition">
                ...
                >
                    ...
                    <span className={`ds-title font-medium text-foreground tracking-tight`}>Jadwal CE Gardu Induk</span>
                                            ...
                                            {todaySchedules.map((r, i) => (
                                                <li key={i} className="px-4 py-2 flex justify-between items-center hover:bg-ds-hover ds-transition">
                                                    <div className="flex flex-col max-w-[70%]">
                                                        ...
                                                        <span className="ds-small truncate">{r[H.BAY]}</span>
                                            ...
                                            {overdueSchedules.map((r, i) => (
                                                <li key={i} className="px-4 py-2 flex justify-between items-center hover:bg-ds-hover ds-transition">
                                                    <div className="flex flex-col max-w-[65%]">
                                                        ...
                                                        <span className="ds-small truncate">{r[H.PROGRAM]}</span>
                                                    </div>
            ...
            {/* 2 Donut Charts — identik pattern CE Transmisi */}
