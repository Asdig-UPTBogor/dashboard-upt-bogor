    ...
    return (
        <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-[#0a0a1a] via-[#111133] to-[#0a0a1a] flex flex-col items-center justify-center text-white select-none">
            ...
            <div className="relative w-full max-w-xl h-64 mb-6">
                ...
                <div className="gundam-fighter absolute bottom-6 right-8 sm:right-16" style={{ transform: "scaleX(-1)" }}>
                    <svg viewBox="0 0 100 160" className="w-20 h-32 sm:w-28 sm:h-44" overflow="visible">
                        ...
                        {/* Right arm — beam rifle (reaches across toward Godzilla) */}
                        <g className="gd-rifle-arm">
                            ...
                            {/* AK-47 bullets — rapid fire toward Godzilla! */}
                        ...
                        {/* Left arm — beam saber only (faces Godzilla!) */}
            ...
            <style jsx>{`
                /* === 8-SECOND FIGHT LOOP === */
                /* 
                 * Phase 1 (0-12%): Ready stance / approach
                 * Phase 2 (12-25%): Rush in, CLASH!
                 * Phase 3 (25-37%): Gundam uppercut
                 * Phase 4 (37-50%): Godzilla tail whip
                 * Phase 5 (50-62%): Gundam beam saber slash
                 * Phase 6 (62-75%): Godzilla atomic breath
                 * Phase 7 (75-87%): Final clash
                 * Phase 8 (87-100%): Reset
                 */

                .godzilla-fighter {
                    animation: gz-fight 8s ease-in-out infinite;
                }
                .gundam-fighter {
                    animation: gd-fight 8s ease-in-out infinite;
                }
                .impact-fx {
                    animation: impact-fight 8s ease-in-out infinite;
                }
                .hit-sparks {
                    animation: sparks-fight 8s ease-in-out infinite;
                }

                /* Godzilla fight movement */
                @keyframes gz-fight {
                ...
                /* Gundam fight movement (mirrored) */
                @keyframes gd-fight {
                ...
                /* Impact flash — appears during clash moments */
                ...
                /* Spark lines — shoot out during impacts */
                @keyframes sparks-fight {
                    0%   { opacity: 0; }
                    14%  { opacity: 0; }
                    15%  { opacity: 1; }
                    19%  { opacity: 0; }
                ...
                /* Hit text POW! BOOM! */
                .hit-text {
                    animation: pow-text 8s ease-in-out infinite;
                }
                .hit-text2 {
                    animation: boom-text 8s ease-in-out infinite;
                }
                @keyframes pow-text {
                    0%, 14%, 18%, 36%, 40%, 100% { opacity: 0; transform: scale(0.5) translateY(0); }
                    15% { opacity: 1; transform: scale(1.5) translateY(-10px); }
                    17% { opacity: 0.5; transform: scale(1.8) translateY(-20px); }
                    37% { opacity: 1; transform: scale(1.3) translateY(-8px); }
                    39% { opacity: 0.5; transform: scale(1.6) translateY(-18px); }
                }
                @keyframes boom-text {
                    0%, 21%, 26%, 49%, 54%, 79%, 84%, 100% { opacity: 0; transform: scale(0.5) translateY(0); }
                    22% { opacity: 1; transform: scale(1.4) translateY(-12px); }
                    25% { opacity: 0.3; transform: scale(1.7) translateY(-22px); }
                    50% { opacity: 1; transform: scale(1.3) translateY(-8px); }
                    53% { opacity: 0.3; transform: scale(1.6) translateY(-18px); }
                    80% { opacity: 1; transform: scale(1.8) translateY(-10px); }
                    83% { opacity: 0.3; transform: scale(2.0) translateY(-20px); }
                }

                /* Godzilla breath — visible during breath phase */
                .gz-breath {
                    animation: breath-vis 8s ease-in-out infinite;
                }
                @keyframes breath-vis {
                ...
                /* Beam saber slash animation — swings during melee phases */
                .gd-saber-arm {
                    animation: saber-slash 8s ease-in-out infinite;
                }
                @keyframes saber-slash {
                    /* Idle — arm down */
                    0%   { transform: rotate(0deg); }
                    6%   { transform: rotate(0deg); }
                    /* CLASH 1 — overhead swing DOWN! */
                    ...
                    /* Tail whip dodge — pull back */
                    35%  { transform: rotate(20deg); }
                    38%  { transform: rotate(0deg); }
                    /* Saber dash — big forward slash! */
                    47%  { transform: rotate(70deg); }
                    50%  { transform: rotate(-45deg); }
                    52%  { transform: rotate(10deg); }
                    54%  { transform: rotate(0deg); }
                    /* Dodge breath — guard position */
                    60%  { transform: rotate(30deg); }
                    65%  { transform: rotate(0deg); }
                    /* Final mega clash — double slash!! */
                    76%  { transform: rotate(65deg); }
                ...
                /* Slash trail — visible during slash moments */
                .gd-slash-trail {
                    animation: slash-trail 8s ease-in-out infinite;
                }
                @keyframes slash-trail {
                    0%, 12%, 17%, 19%, 24%, 46%, 52%, 75%, 84%, 100% { opacity: 0; }
                    15% { opacity: 0.8; }
                    22% { opacity: 0.7; }
                    50% { opacity: 0.9; }
                    78% { opacity: 0.8; }
                ...
                /* Gundam rifle — visible during shooting phase */
                .gd-muzzle, .gd-bullet1, .gd-bullet2, .gd-bullet3, .gd-bullet4, .gd-bullet5,
                .gd-bullet6, .gd-bullet7, .gd-bullet8, .gd-bullet9, .gd-bullet10 {
                    animation: bullet-vis 8s ease-in-out infinite;
                }
                .gd-bullet2 { animation-delay: 0.08s; }
                .gd-bullet3 { animation-delay: 0.16s; }
                .gd-bullet4 { animation-delay: 0.24s; }
                .gd-bullet5 { animation-delay: 0.32s; }
                .gd-bullet6 { animation-delay: 0.40s; }
                .gd-bullet7 { animation-delay: 0.48s; }
                .gd-bullet8 { animation-delay: 0.56s; }
                .gd-bullet9 { animation-delay: 0.64s; }
                .gd-bullet10 { animation-delay: 0.72s; }
                @keyframes bullet-vis {
                    /* Bullets visible during Gundam's attack phases */
                    0%, 11%, 26%, 35%, 55%, 69%, 86%, 100% { opacity: 0; }
                    /* Phase: Gundam rushes in firing */
                    12% { opacity: 1; }
                    25% { opacity: 0.8; }
                    /* Phase: Beam saber + shooting */
                    42% { opacity: 1; }
                    54% { opacity: 0.8; }
                ...
                .gd-muzzle {
                    animation: muzzle-flash 0.15s steps(2) infinite;
                }
                ...
                /* Screen shake during big impacts */
                .shake-overlay {
                    animation: screen-shake 8s ease-in-out infinite;
                }
                @keyframes screen-shake {
                    0%, 14%, 16%, 21%, 24%, 36%, 39%, 79%, 83%, 100% { transform: translate(0, 0); }
                    15%  { transform: translate(-3px, 2px); }
                    15.5% { transform: translate(3px, -2px); }
                    ...
                    80.5% { transform: translate(4px, -2px); }
                    81%  { transform: translate(-2px, 4px); }
                    81.5% { transform: translate(3px, -3px); }
                }
            `}</style>
        </div>
    );
