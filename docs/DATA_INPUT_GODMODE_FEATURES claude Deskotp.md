# Data Input Workspace — True God Mode Feature List

> Semua fitur yang harus ada agar Data Input Workspace menjadi
> sistem data management yang config-driven, zero-coding, production-grade.

---

## Level 1: Core (Foundation — HARUS ADA)
...
## Level 2: Smart Features (Productivity Boost)
...
## Level 3: Intelligence (Config-Driven Logic)

### N. Formula Engine
```
N01. Calculated columns (formula dari kolom lain)
     Contoh: "total_harga" = "{harga_satuan}" × "{jumlah}"
N02. Date formulas
     Contoh: "umur_aset" = TODAY() - "{tanggal_operasi}"
N03. Conditional value
     Contoh: IF "{umur_aset}" > 25 THEN "Perlu Penggantian"
N04. Cross-table formula
     Contoh: COUNT rows di Asset_Data.Relay WHERE bay_id = THIS.id
N05. Aggregation formula
     Contoh: SUM("{daya_mva}") WHERE gi_id = THIS.gi_id
...
### O. Conditional Rules
```
O01. Conditional required
     IF tipe_bay = "Trafo" THEN daya_mva REQUIRED
O02. Conditional visibility
     IF tipe_bay = "Trafo" THEN SHOW kolom daya_mva
O03. Conditional dropdown filter
     IF brand = "NR" THEN type OPTIONS = [PCS-9611, PCS-9612]
O04. Conditional formatting
     IF status = "Rusak" THEN cell background merah
O05. Conditional default value
     IF tipe_bay = "Busbar" THEN default tegangan = "150"
...
```
P01. One-to-many display
     Di tabel Bay → section "Assets di Bay ini" (list Relay, CT, PT)
P02. Clickable reference
     Klik "GI 150KV SENTUL" di tabel Bay → jump ke tabel GI, row itu
P03. Back-reference count
     Di tabel GI → kolom "Total Bay: 12"
P04. Related data preview
     Hover reference → popup preview data dari tabel master
P05. Dependency graph
     Visualisasi: GI → Bay → Relay/CT/PT (tree view)
P06. Impact analysis
     "Kalau delete GI ini, 12 Bay dan 45 Asset akan terpengaruh"
```

### Q. Views & Saved Filters
```
Q01. Save filter combination sebagai "View"
     Contoh: "Relay NR di ULTG Bogor" = filter brand=NR + ultg=BOGOR
...
## Level 4: Enterprise (Multi-User & Control)
...
## Level 5: God Mode (Self-Operating System)
...
```
Level 1 (Core)           : A8 + B10 + C10 + D6 + E10 + F7 + G4  = 55 fitur
Level 2 (Smart)          : H11 + I6 + J7 + K5 + L7 + M6          = 42 fitur
Level 3 (Intelligence)   : N8 + O7 + P6 + Q7                     = 28 fitur
Level 4 (Enterprise)     : R6 + S6 + T6 + U5                     = 23 fitur
Level 5 (God Mode)       : V7 + W6 + X6 + Y8 + Z6 + AA6          = 39 fitur
─────────────────────────────────────────────────────────────────────
TOTAL                                                             = 187 fitur
```
...
## Roadmap Timeline (Estimasi)

```
Level 1 (Core)           : Sprint 1-3    (3 minggu)   ← PRIORITAS SEKARANG
Level 2 (Smart)          : Sprint 4-6    (3 minggu)
Level 3 (Intelligence)   : Sprint 7-10   (4 minggu)
Level 4 (Enterprise)     : Sprint 11-14  (4 minggu)
Level 5 (God Mode)       : Sprint 15-20  (6 minggu)
─────────────────────────────────────────────────────
TOTAL                    : ~20 sprint    (~5 bulan)
```
...
MUST HAVE (tanpa ini ga bisa dipakai):
  Level 1: A01-A03, B01-B04, B09, C01-C04, C06,
           D01-D03, E01-E07, F01-F07, G01

SHOULD HAVE (bikin experience bagus):
  Level 1: A05, B05-B08, C05, C07-C10, D04-D06, E08-E09
  Level 2: H01-H10, I01-I03, L01-L05, M01-M04

NICE TO HAVE (upgrade bertahap):
  Level 2: I04-I06, J01-J07, K01-K05, L06-L07, M05-M06
  Level 3: semua
  Level 4: semua
  Level 5: semua
```
